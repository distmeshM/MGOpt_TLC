...
{
    if (nrhs < 3) 
        mexErrMsgTxt("Invalid input: not enough input, idx = ij2nzIdxs(A, i, j);");
    ...
    plhs[0] = mxDuplicateArray(mat_I);
    size_t *pnzIdx = (size_t*)mxGetData(plhs[0]);

    const auto intmax = std::numeric_limits<size_t>::max();

    const int offset = 1; // for matlab 1-based index
#pragma omp parallel for shared(A_Ir, A_Jc, pI, pJ, pnzIdx)
    for (int i = 0; i < k; i++) {
        ...
        pnzIdx[i] = (it == pend) ? intmax : it - A_Ir+offset;
    }
}
