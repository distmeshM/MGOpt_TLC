...
bool SSP_collapse_edge(
  ...
  Eigen::VectorXi & FIdx_onering_pre, std::vector<bool>& isB)
{
  ...
  //检测“耳部边”生成
  bool earEdge = false;
  int s1 = E(e, 0), d1 = E(e, 1);
  if (isB[s1] && !isB[d1])
  {
      for (int i = 0; i < Nsv_alec.size(); i++)
      {
          ...
          if (isB[Nsv_alec[i]])
          {
              ...
              {
                  bool neigh = false;
                  for (int j = 0; j < Ndv_alec.size(); j++)
                  {
                      if (Nsv_alec[i] == Ndv_alec[j])
                      {
                          neigh = true;
                      }
                  }
  ...
  if (isB[d1] && !isB[s1])
  {
      for (int i = 0; i < Ndv_alec.size(); i++)
      {
          if (isB[Ndv_alec[i]])
          {
              ...
              {
                  bool neigh = false;
                  for (int j = 0; j < Nsv_alec.size(); j++)
                  {
                      if (Ndv_alec[i] == Nsv_alec[j])
                      {
                          neigh = true;
                      }
                  }
...
bool SSP_collapse_edge(
  ...
  int & f2,
  std::vector<single_collapse_data> & decInfo,
  std::vector<std::vector<int>> & decIM, std::vector<bool>& isB)
{
  using namespace Eigen;
  using namespace igl;
  using namespace std;
  ...
  if(pre_collapse(V,F,E,EMAP,EF,EI,Q,EQ,C,e))
  {
    collapsed = SSP_collapse_edge(
      e,C.row(e),
      Nsv,Nsf,Ndv,Ndf,
      V,F,E,EMAP,EF,EI,e1,e2,f1,f2,decInfo,decIM,data,FIdx_onering_pre,isB);
  }else
  {
    collapsed = false;
  }
