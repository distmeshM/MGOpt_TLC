	...
	VectorXb& constrained, double w1)
{
	...
	for (int lv = 1; lv < nLvs; lv++)
	{
		...
		MatrixXd Vc;
		MatrixXi Fc;
		get_prolong(V,F,tarF, dec_type, Vc,Fc,P,P2,constrained, w1);
	...
	VectorXb& constrained)
{
	...
	mg_precompute(Vf,Ff,decimation_type, mg, constrained);
}
	...
	VectorXb& constrained)
{
	...
	mg_precompute(Vf,Ff,coarsening_ratio,min_coarsest_nV,dec_type, mg, constrained, 0.01);
}