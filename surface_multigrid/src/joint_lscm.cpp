...
void flatten(
	...
	const int& nVjoint,
	const bool& isDebug,
	Eigen::VectorXd& UVjoint_flat, int nonlinearId)
{
	using namespace Eigen;
	using namespace std;
...
void flatten(
	...
	const int& nVjoint,
	const bool& isDebug,
	Eigen::VectorXd& UVjoint_flat, int nonlinearId, int dir)
{
	using namespace Eigen;
	using namespace std;
	...
	// solve UV jointly
	{
		...
		{
			//mqwf_constraint_solve(Q, RHS, b_UV, bc_UV, UVjoint_flat, nonlinearId, dir);
			mqwf_nonlinear_precompute(Q, b_UV, data, nonlinearId, dir);
			mqwf_nonlinear_solve(data, RHS, bc_UV, UVjoint_flat, dir);
		}
...
void flatten(
	...
	const int& nVjoint,
	const bool& isDebug,
	Eigen::VectorXd& UVjoint_flat, Eigen::VectorXi nonlinearIds)
{
	using namespace Eigen;
	using namespace std;
	...
	// solve UV jointly
	{
		...
		mqwf_nonlinear_precompute(Q, b_UV, data, nonlinearIds);
		mqwf_nonlinear_solve(data, RHS, bc_UV, UVjoint_flat, nonlinearIds);
	...
	Eigen::MatrixXd& UV_post)
{
	...
	{
		// PROFC_NODE("lscm: solve");
		flatten(Vjoint_pre, Fjoint_pre, Vjoint_post, Fjoint_post, b_UV, bc_UV, nVjoint, isDebug, UVjoint_flat, -1);
	}
	...
	Eigen::MatrixXd& UV_post)
{
	...
	int vk = case2_constraint3_snap1(V_pre, FUV_pre, V_post, FUV_post, vi, vj, bdLoop, verbose, isDebug, onBd, vi, UV_pre_vi, UV_post_vi, 1);
	...
	double doti = (UV_pre_vi.row(vk) - UV_pre_vi.row(vj)).dot(UV_pre_vi.row(vj) - UV_pre_vi.row(vi));
	if (doti <= 0)
	{
		objVal_snap_vi = std::numeric_limits<int>::max();
	}
	...
	vk = case2_constraint3_snap1(V_pre, FUV_pre, V_post, FUV_post, vi, vj, bdLoop, verbose, isDebug, onBd, vj, UV_pre_vj, UV_post_vj, 0);
	...
	// }
	double dotj = (UV_pre_vj.row(vk) - UV_pre_vj.row(vi)).dot(UV_pre_vj.row(vi) - UV_pre_vj.row(vj));
	...
	std::tuple<int, int> vkl = case2_constraint4(V_pre, FUV_pre, V_post, FUV_post, vi, vj, bdLoop, verbose, isDebug, onBd, UV_pre_nosnap, UV_post_nosnap);
	vk = std::get<0>(vkl);
	int vl = std::get<1>(vkl);
	double x0 = UV_pre_nosnap(vk, 1);
	double x1 = UV_pre_nosnap(vi, 1);
	double x2 = UV_pre_nosnap(vj, 1);
	double x3 = UV_pre_nosnap(vl, 1);
	...
	int dir)
{
	...
	{
		// PROFC_NODE("lscm: solve");
		flatten(Vjoint_pre, Fjoint_pre, Vjoint_post, Fjoint_post, b_UV, bc_UV, nVjoint, isDebug, UVjoint_flat, vk, dir);
	}
	if (verbose)
		cout << "finish solving \n";
...
std::tuple<int, int> case2_constraint4(
	const Eigen::MatrixXd& V_pre,
	const Eigen::MatrixXi& FUV_pre,
	const Eigen::MatrixXd& V_post,
	const Eigen::MatrixXi& FUV_post,
	...
	Eigen::MatrixXd& UV_post)
{
	...
	{
		// PROFC_NODE("lscm: solve");
		flatten(Vjoint_pre, Fjoint_pre, Vjoint_post, Fjoint_post, b_UV, bc_UV, nVjoint, isDebug, UVjoint_flat, neighbor_to_vi);
	}
	if (verbose)
		cout << "finish solving \n";
	...
	return std::tuple<int, int>(neighbor_to_vi(0), neighbor_to_vi(2));
}
