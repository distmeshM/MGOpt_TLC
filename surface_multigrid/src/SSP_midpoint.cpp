  ...
  Eigen::VectorXi & FIM)
{
  ...
  std::vector<bool> isB = igl::is_border_vertex(F);
  Eigen::VectorXi EMAP;
...
bool SSP_midpoint(
  ...
  std::vector<single_collapse_data> & decInfo,
  std::vector<std::vector<int>> & decIM,
  Eigen::VectorXi & FIM, std::vector<bool>& isB)
{
  decimate_pre_collapse_func always_try;
  decimate_post_collapse_func never_care;
  always_try_never_care(always_try,never_care);
  return SSP_midpoint(
    OV,OF,cost_and_placement,stopping_condition,always_try,never_care,U,G,J,I,decInfo, decIM, FIM, isB);
}
  ...
  Eigen::VectorXi & FIM, std::vector<bool>& isB)
{
  ...
  while(true)
  {
    ...
    if(SSP_collapse_edge(
      ...
      V,F,E,EMAP,EF,EI,Q,EQ,C,e,e1,e2,f1,f2,decInfo,decIM,isB))
    {
    ...
    }else
    {
      if(prev_e == e)
      {
          cout << "Edge collapse no progress... bad stopping condition?";
