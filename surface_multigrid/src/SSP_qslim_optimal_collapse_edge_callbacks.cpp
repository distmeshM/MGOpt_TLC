    ...
    VectorXb& constrained)
{
  ...
  cost_and_placement = [&quadrics,&v1,&v2,&constrained](
    const int e,
    ...
    Eigen::RowVectorXd & p)
  {
    ...
    //mexPrintf("%d\n", 1);
    if ((!constrained(E(e, 0))) && (!constrained(E(e, 1))))
    {
        p = -b * A.inverse();
        cost = p.dot(p * A) + 2 * p.dot(b) + c;
    }
    else if(constrained(E(e, 0)) && (!constrained(E(e, 1))))
    {
        //p = V.row(E(e, 0));
        //cost = p.dot(p * A) + 2 * p.dot(b) + c;
  ...
  // update quadric
  post_collapse = [&v1,&v2,&quadrics,&constrained](
      const Eigen::MatrixXd &                             ,   /*V*/
      const Eigen::MatrixXi &                             ,   /*F*/
      const Eigen::MatrixXi &                             ,   /*E*/
      const Eigen::VectorXi &                             ,/*EMAP*/
      ...
      )->void
  {
    if(collapsed)
    {
        bool v12 = constrained[v1] || constrained[v2];
        constrained[v1] = v12; constrained[v2] = v12;
