	...
	VectorXb& constrained)
{
	...
	mg_precompute_block(Vf,Ff,decimation_type, mg, constrained);
}
	...
	VectorXb& constrained)
{
	...
	mg_precompute_block(Vf,Ff,coarsening_ratio,min_coarsest_nV,dec_type, mg, constrained);
}
	...
	VectorXb& constrained)
{
	...
	for (int lv = 1; lv < nLvs; lv++)
	{
		...
		MatrixXd Vc;
		MatrixXi Fc;
		get_prolong_block(V,F,tarF, dec_type, Vc,Fc,P,constrained);
