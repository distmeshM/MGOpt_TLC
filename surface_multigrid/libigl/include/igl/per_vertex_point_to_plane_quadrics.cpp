...
IGL_INLINE void igl::per_vertex_point_to_plane_quadrics(
  ...
  std::vector<
    std::tuple<Eigen::MatrixXd,Eigen::RowVectorXd,double> > & quadrics, double w1)
{
  using namespace std;
  typedef std::tuple<Eigen::MatrixXd,Eigen::RowVectorXd,double> Quadric;
  const int dim = V.cols();
  std::vector<Eigen::RowVector3d> vertexNormal;
  std::vector<double> ai;
  ai.resize(V.rows(), 0);
  vertexNormal.resize(V.rows(), Eigen::RowVector3d::Zero(dim));
  //// Quadrics per face
  ...
  for(int f = 0;f<F.rows();f++)
  {
    ...
    if(infinite_corner == -1)
    {
      ...
      for(int c = 0;c<3;c++)
      {
        ...
        if (dim == 3)
        {
            Eigen::RowVector3d E1 = e1;
            Eigen::RowVector3d E2 = e2;
            //Eigen::RowVector3d E3 = E1.cross(E2);
            vertexNormal[F(f, c)] = vertexNormal[F(f, c)] + area * E1.cross(E2);
        }
        ai[F(f, c)] = ai[F(f, c)] + area / 3;
      }
    }else
    {
  ...
  for (int v = 0; v < V.rows(); v++)
  {
      vertexNormal[v] = vertexNormal[v] / vertexNormal[v].norm();
  }
  for (int v = 0; v < V.rows(); v++)
  {
      Eigen::RowVector3d thisNormal = vertexNormal[v];
      Eigen::RowVector3d vV = V.row(v);
      Eigen::MatrixXd A = I;
      Eigen::RowVectorXd b = -vV;
      double c = vV.dot(vV);

      A += -thisNormal.transpose() * thisNormal;
      b += thisNormal.dot(vV) * thisNormal;
      c += -pow(vV.dot(thisNormal), 2);
      double weight = w1 * ai[v];
      quadrics[v] = quadrics[v] + Quadric{ weight * A, weight * b, weight * c };
  }
}
